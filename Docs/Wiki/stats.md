| home | score (0-0) | away |
|----:|:-----------:|:----|
| earliest scorer's name (comma separated, minutes of all their goals, ascending with a flag of if it was from penalty)  |  | Sessay 21', 30'(p), 90'+4' |
| second earliest scorer's name (minutes of all their goals, ascending)  |  | Vladoiu (25', 30', 35', 60', 72') |

| home | | away |
|:----:|:-----------:|:----:|
|      | ATTACKING         |      |
| 0  | Total Shots | 0 |
| 0 / 0  | On Target / Corners     | 0 / 0  |
| 0 / 0  | Blocked / Off Target  | 0 / 0  |
|      | PASSING        |      |
| 0 | Asissts   | 0  |
| 0 / 0  | Ground (Att./Made)    | 0 / 0  |
| 0 / 0 / 0| Aerial(Att./Trg./Made)   | 0 / 0 / 0|
|      | Game Play        |      |
| paces     | Distance Covered |  paces    |
| 50%  | Possession  | 50%  |
|      | DUELS        |      |
| 0 / 0  | Ground Att. / Won    | 0 / 0  |
| 0 / 0  | Air Att. / Won    | 0 / 0  |
|      | DISCIPLINE        |      |
| 0 / 0  | Yellow / Red Cards  | 0 / 0  |
| 0 / 0  | Injuries / Subs Used   | 0 / 0  |
|      | OPTA STATS |      |
| 0.00 / 0 | xRecoveries / Made        | 0.00 / 0 |
| 0.00 / 0 | xDribbles / Made      | 0.00 / 0 |
| 0.00 / 0 | xTackles / Made       | 0.00 / 0 |

| home team | XI | away team |
|---------:|:-----------:|:---------|
| **Aziz** | **1** | **Kuzmic** |
| **playername** | **2** | **playername** |
| **playername** | **3** | **playername** |
...
| **playername** | *11* | **playername** |
| playername | 12 | playername |
...
| playername | 16-18 | playername |
